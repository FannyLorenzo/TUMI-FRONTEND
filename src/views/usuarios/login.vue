          ...
          <v-row>
            ...
            <v-col lg="5">
              <div class="pa-7 pa-sm-12">
                ...
                <h2 class="font-weight-bold mt-4 white--text text--darken-2">Iniciar sesión</h2>
                <h6 class="subtitle-1">
...
items: [
  ...
  ...mapState("auth", ["current_user","current_id","token"]),
    ...
    Usuario_Actual: {
      get() {
        return this.$store.state.usuario_Actual;
      },
      set(val) {
        this.$store.commit("SET_USUARIO_ACTUAL", val);
      },
    },
  },
  methods: {
    ...
    submit() {
      ...
      axios.post('https://capacitacion-docente.herokuapp.com/login/', json)
      .then(data =>{
            console.log("entroo");
            ...
            this.Usuario_Actual = data.data.usuario;
            this.SET_CURRENT_ID(data.data.usuario.usuario_id);
            localStorage.setItem("token ", data.data.usuario.usuario_id);
            this.token = data.data.usuario.usuario_id;
