    ...
    return {
        ...
        processItems: function(items, change) {
            ...
            for (var i = 0; i < editData.length; i++) {
                ...
                if (key && editData[i].type === DATA_EDIT_DATA_INSERT_TYPE) {
                    var loadedRowIndex = this._getLoadedRowIndexByInsertKey(items, change, key);
        ...
        saveEditData: function() {
            ...
            var afterSaveEditData = function(cancel) {
                (0, _deferred.when)(_this8._afterSaveEditData(cancel)).done(function() {
                    deferred.resolve()
                })
            };
            _deferred.when.apply(void 0, _toConsumableArray(this._deferreds)).done(function() {
        ...
        _saveEditDataInner: function() {
            ...
            var resetModifiedClassCells = function() {
                if (editMode === EDIT_MODE_BATCH) {
                    var columnsCount = _this9._columnsController.getVisibleColumns().length;
                    editData.forEach(function(_ref) {
                        var key = _ref.key;
                        var rowIndex = _this9._dataController.getRowIndexByKey(key);
                        if (rowIndex !== -1) {
                            for (var columnIndex = 0; columnIndex < columnsCount; columnIndex++) {
                                _this9._rowsView._getCellElement(rowIndex, columnIndex).removeClass(CELL_MODIFIED)
                            }
                        }
                    })
                }
        ...
        closeEditCell: function(isError, withoutSaveEditData) {
            ...
            if (!_isRowEditMode(that)) {
                ...
                _deferred.when.apply(void 0, _toConsumableArray(this._deferreds)).done(function() {
                    _this10._closeEditCellCore(isError, oldEditRowIndex, withoutSaveEditData);
                    result.resolve()
                }).fail(result.reject)
            }
            return result.promise()
